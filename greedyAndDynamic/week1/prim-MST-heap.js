...
const fileName = "edge";
fs.readFile(
    `./greedyAndDynamic/week1/${fileName}.txt`,
    "utf-8",
    (err, data) => {
...
function primMSTWithHeap(graph, source) {
    ...
    while (heap.length > 0) {
        ...
        x.add(key);
        T.push(value);
        const toRefreshList = adjList.get(key);
        toRefreshList.forEach((item) => {
            const { target, score } = item;
            if (!x.has(target)) {
                const oldValue = R.find(R.propEq("key", target), heap).value;
                if (oldValue > score) {
                    heap = deleteNode(heap, target);
                    const item = { key: target, value: score };
                    heap = heapInsert(heap, item);
                }
            }
        });
...
function minHeapfy(arr, length, i) {
    ...
    if (
        leftChild < length &&
        arr[leftChild].value < arr[tempLargestPosition].value
    ) {
        tempLargestPosition = leftChild;
    }
    if (
        rightChild < length &&
        arr[rightChild].value < arr[tempLargestPosition].value
    ) {
...
function extractMin(heap) {
    if (heap.length <= 1) {
        return { min: heap[0], newHeap: [] };
    }
    const copyHeap = [...heap];
    const min = copyHeap[0];
    const newHeap = [
        copyHeap[copyHeap.length - 1],
        ...copyHeap.slice(1, copyHeap.length - 1),
    ]; // tail to head
    const _help = (heap, length, i) => {
        ...
        if (
            leftChild < length &&
            rightChild < length &&
            heap[leftChild].value < heap[tempIndex].value &&
            heap[rightChild].value < heap[tempIndex].value
        ) {
...
function deleteNode(heap, node) {
    if (!heap.find(({ key }) => node === key)) {
        return heap;
    }
    const index = heap.findIndex((ele) => ele.key === node);
